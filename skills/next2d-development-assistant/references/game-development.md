...
3. ViewModelはゲーム操作と更新時刻を管理し、Viewは描画と入力の購読・解除だけを担う。`Event.ENTER_FRAME` は `onEnter` で登録し `onExit` で解除する。
...
- 長時間ファイル更新がない実行は中断し、最後に成功した状態から小さな指示で再開する。モデルの応答待ちを「実装済み」と扱わない。

## 検証の順序

1. 変更前に `npx tsc --noEmit --skipLibCheck` と `npm test -- --run` を実行し、既存エラーを記録する。
2. ドメインテストを単独で通してから、プロジェクト全体の型検査・Webビルドを実行する。
3. Playwrightでは canvas の有無、console/page error、入力後の状態を確認する。headless Chromiumで `WebGPU adapter not available` が出てcanvasが1×1のままなら、ゲームの失敗とは断定せずWebGPU対応ブラウザで再確認する。
