...
- LocalLLMは規模にかかわらず数時間かかることがある。長工程の開始前や再起動後は、読み取り専用の短いprobeでモデル応答を確認する。
- CLIの無出力やファイル更新の停止だけでは中断しない。セッションDBの`tokens_output` / `tokens_reasoning`、tool call、`time_idle`も確認し、意味のあるreasoningまたはトークンが増えている間は待機する。
- `time_idle`後も成果物・tool call・意味のある出力がない、reasoningが空または同じ文字列を反復する、出力上限へ達した場合は明確な失敗として中断する。再依頼は機械的な1変更に縮小し、探索が不要なら完全な置換内容またはold/new文字列を渡す。
